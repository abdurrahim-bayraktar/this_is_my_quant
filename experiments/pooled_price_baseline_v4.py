"""
Pooled Price-Only Baseline V4 - Anti-Overfitting + More Data.

Changes from V3:
1. Lower learning rate (1e-4 instead of 1e-3)
2. More dropout (0.5 instead of 0.3)
3. Patience = 20 (instead of 50)
4. Double stocks (400 instead of 200)
5. Price caching for faster re-runs

Usage:
    python experiments/pooled_price_baseline_v4.py --epochs 100 --stocks 400
"""
...
# Extended stock list - S&P 500 + additional major stocks
EXTENDED_TICKERS = TOP_200_TICKERS + [
    # Additional S&P 500 stocks not in TOP_200
...
class PriceCache:
    """Cache for yfinance price data to avoid re-downloading."""
...
class PooledExperimentV4:
    """V4: Anti-overfitting + more data."""
    ...
    def __init__(
        ...
        epochs: int = 100,
        batch_size: int = 1024,
        early_stopping_patience: int = 20,  # V4: Reduced from 50
        dropout: float = 0.5,               # V4: Increased from 0.3
        learning_rate: float = 1e-4,        # V4: Reduced from 1e-3
        use_cache: bool = True,
    ):
        self.stocks = stocks or EXTENDED_TICKERS[:400]  # V4: More stocks
        ...
        self.output_dir = REPORTS_DIR / f"pooled_v4_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def load_all_stocks(self) -> Dict[str, pd.DataFrame]:
        ...
        if self.use_cache:
            ...
            if cached:
                ...
                if len(filtered) >= len(self.stocks) * 0.8:  # At least 80% found
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray) -> Dict:
        ...
        model = AttentionLSTM(
            ...
            dropout=self.dropout,  # V4: 0.5 instead of 0.3
        )
        ...
        trainer = Trainer(
            model, loss_fn,
            learning_rate=self.learning_rate,  # V4: 1e-4 instead of 1e-3
            weight_decay=1e-4,
        )
    ...
    def run(self):
        """Run the experiment."""
        logger.info("=" * 80)
        logger.info("POOLED BASELINE V4: Anti-Overfitting + More Data")
        logger.info("=" * 80)
        logger.info(f"  Stocks: {len(self.stocks)}")
        logger.info(f"  Dropout: {self.dropout} (v3: 0.3)")
        logger.info(f"  Learning Rate: {self.learning_rate} (v3: 1e-3)")
        logger.info(f"  Patience: {self.early_stopping_patience} (v3: 50)")
...
def main():
    ...
    parser.add_argument("--stocks", type=int, default=400)  # V4: More stocks
    parser.add_argument("--batch-size", type=int, default=1024)
    parser.add_argument("--patience", type=int, default=20)  # V4: Lower patience
    parser.add_argument("--dropout", type=float, default=0.5)  # V4: Higher dropout
    parser.add_argument("--lr", type=float, default=1e-4)  # V4: Lower LR
