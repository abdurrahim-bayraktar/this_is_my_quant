"""
Regression DA-RNN — Walk-Forward (Rolling-Origin) Validation.

Extends regression_darnn.py by replacing the static train/val/test split with
walk-forward validation that simulates realistic deployment:
  - Each fold trains on ALL data up to the fold's start (expanding window)
  - Validates on a single quarter (e.g., Q1 2023)
...
the model is trained once on 2014–2022 and then deployed over a long
unseen future (2023–2024).
...
Key design decisions:
  ...
  - Walk-forward folds: quarterly expanding window (default 6 folds over 2023–2024)

Usage:
    python experiments/regression_darnn_rolling.py --epochs 100 --stocks 400
    python experiments/regression_darnn_rolling.py --epochs 2 --stocks 5  # smoke test
    python experiments/regression_darnn_rolling.py --wf-step QS  # quarterly (default)
    python experiments/regression_darnn_rolling.py --wf-step MS  # monthly
"""
...
# ============================================================================
# EXPERIMENT — Walk-Forward DA-RNN
# ============================================================================

class DARNNRollingExperiment:
    """
    Walk-forward DA-RNN regression experiment: predict vol-adjusted forward returns.
    ...
    4. Pre-compute features for all tickers (done once, reused across folds)
    ...
    def __init__(self, config: Dict):
        ...
        self.wf_val_start = pd.Timestamp(config.get("wf_val_start", "2023-01-01"))
        self.wf_val_end = pd.Timestamp(config.get("wf_val_end", "2024-12-31"))
        ...
        # Output directory
        exp_name = config.get("experiment_name", "wf_regression_darnn")
    ...
    def _generate_walk_forward_folds(self) -> List[Tuple[pd.Timestamp, pd.Timestamp]]:
        ...
        With QS (quarterly): ~8 folds over 2023–2024
        With MS (monthly):   ~24 folds over 2023–2024
        """
    ...
    def prepare_all_stock_features(
        ...
        for ticker in tqdm(tickers, desc="Computing features"):
            if ticker not in stock_data:
                continue
            try:
                result = self._compute_features_and_target(stock_data[ticker])
                if result[0] is None:
                    continue
    ...
    def run(self):
        """Execute the walk-forward DA-RNN regression experiment pipeline."""
        folds = self._generate_walk_forward_folds()

        logger.info("=" * 80)
        logger.info("WALK-FORWARD DA-RNN REGRESSION EXPERIMENT — Volatility-Adjusted Forward Return")
        logger.info("=" * 80)
        ...
        summary = {
            "experiment_name": self.config.get("experiment_name", "wf_regression_darnn"),
            "task": "regression",
            "model": "DARNN",
...
def main():
    parser = argparse.ArgumentParser(description="Walk-Forward DA-RNN Regression — Vol-Adjusted Forward Return")
    ...
    parser.add_argument("--name", type=str, default="wf_regression_darnn", help="Experiment name")
    # Date range
    parser.add_argument("--start-date", type=str, default="2014-01-01")
    parser.add_argument("--end-date", type=str, default="2024-12-31")
    # Walk-forward params
    parser.add_argument("--wf-val-start", type=str, default="2023-01-01",
                        help="First fold validation start (default: 2023-01-01)")
    parser.add_argument("--wf-val-end", type=str, default="2024-12-31",
                        help="Last fold validation end (default: 2024-12-31)")
    parser.add_argument("--wf-step", type=str, default="QS",
                        choices=["MS", "QS"],
                        help="Fold frequency: MS=monthly, QS=quarterly (default)")
    ...
    experiment = DARNNRollingExperiment(config)
    experiment.run()
