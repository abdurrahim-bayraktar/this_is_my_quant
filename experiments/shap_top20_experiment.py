"""
SHAP Top 20 Features Experiment - Testing with SHAP-selected features only.

Based on lstm_size_comparison.py with all critical fixes preserved:
...
This experiment uses only the top 20 features identified by SHAP analysis:
  atr_pct, intraday_range, bb_BBB_5_2.0_2.0, gap, cci, high_low_pct,
  volume_ratio, adx_DMN_14, return_1d, aroon_AROOND_14, adx_DMP_14,
  obv, ad, bear_power, roc_10, pvo_PVOh_12_26_9, trix_TRIXs_30_9,
  rsi_14, aroon_AROONU_14, return_5d

Usage:
    python experiments/shap_top20_experiment.py --epochs 100 --stocks 400
"""
...
class ShapTop20Experiment:
    """
    Experiment using only SHAP Top 20 features.
    ...
    def __init__(
        ...
        if colab_mode:
            self.output_dir = Path(f"/content/reports/shap_top20_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        else:
            self.output_dir = REPORTS_DIR / f"shap_top20_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def prepare_stock_with_temporal_split(self, df: pd.DataFrame) -> Tuple[
        ...
        Uses only SHAP Top 20 features computed via compute_shap_top20.
        """
        # Compute only SHAP top 20 features (more efficient)
        ...
        if len(available_features) < 5:
            logger.warning(f"Only {len(available_features)} features available, need at least 5")
            return (np.array([]), np.array([]), np.array([]), 
                    np.array([]), np.array([]), np.array([]))
        
        # Log any missing features (only once)
    ...
    def prepare_temporal_pooled_data(self, stock_data: Dict[str, pd.DataFrame]) -> Tuple[
        ...
        for ticker, df in tqdm(stock_data.items(), desc="Processing stocks with SHAP top 20 features"):
            try:
                ...
                if current_n != n_features:
                    logger.warning(f"{ticker} has {current_n} features, expected {n_features}, skipping")
                    continue
        ...
        logger.info(f"Using {n_features} SHAP top 20 features")
    ...
    def run(self):
        logger.info("=" * 80)
        logger.info("SHAP TOP 20 FEATURES EXPERIMENT")
        logger.info("=" * 80)
        logger.info(f"Features: {SHAP_TOP20_FEATURES}")
        ...
        with open(self.output_dir / "features.txt", "w") as f:
            for col in SHAP_TOP20_FEATURES:
                f.write(f"{col}\n")
...
def main():
    ...
    experiment = ShapTop20Experiment(
        stocks=stocks,
