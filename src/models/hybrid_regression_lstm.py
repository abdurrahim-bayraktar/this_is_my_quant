"""
Hybrid Regression LSTM — Dual-Branch Architecture for Regression.

Regression variant of HybridAttentionLSTM.  Same dual-branch design:
...
Branch 2 (Sentiment): Lightweight single-layer GRU.  Processes the
    2 RF-selected sentiment features through a smaller temporal model
    that respects their lower dimensionality and noisier nature.

Fusion: The final hidden states of both branches are concatenated
    and passed through shared Dense layers before the regression head
    which outputs a single scalar (vol-adjusted forward return).
...
Design choices (identical to classification variant):
...
Regression-specific changes:
    - Output head: Linear(32, 1) → squeeze → scalar prediction
    - No softmax / no num_classes parameter
    - forward() returns (prediction, None, None) for RegressionTrainer
      compatibility
    - predict() returns {"prediction": tensor} matching
      RegressionAttentionLSTM interface
"""
...
class HybridRegressionLSTM(nn.Module):
    """
    Dual-branch architecture for regression: LSTM+Attention for technicals,
    GRU for sentiment, single-scalar output.
    ...
    def __init__(
        ...
        """
        Initialize dual-branch regression model.
        ...
        fusion_params = (
            ...
            + sum(p.numel() for p in self.regression_head.parameters())
        )
        logger.info(
            f"HybridRegressionLSTM: "
    ...
    def _init_weights(self):
        ...
        # Dense layers
        for module in [self.fusion, self.regression_head]:
            for layer in module:
    ...
    def forward(
        ...
        Returns:
            Tuple of (prediction, None, attn_weights_or_None) for
            RegressionTrainer compatibility.
        """
        ...
        tech_attended, attn_weights = self.tech_attention(tech_out)  # (batch, seq_len, tech_hidden)
    ...
    def predict(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dict with "prediction" key containing scalar predictions.
        """
        self.eval()
        with torch.no_grad():
...
def main():
    ...
    print(f"[OK] Shape test passed: {pred.shape}")
    ...
    pred_nosent, _, _ = model(x_nosent)
    assert not torch.isnan(pred_nosent).any(), "NaN in no-sentiment output"
    assert not torch.isinf(pred_nosent).any(), "Inf in no-sentiment output"
    ...
    print(f"[OK] Predict test: prediction shape={preds['prediction'].shape}")
