"""
Comprehensive Technical Indicators Module V7.

Adds domain knowledge binary features on top of V6 indicators:
- Golden/Death Cross signals
- RSI overbought/oversold zones
- MACD crossover signals
- Bollinger Band breakouts
- Volume confirmation signals
- Additional indicators (SAR, CMO, Donchian, etc.)

Usage:
    from src.features.indicators_v7 import ComprehensiveIndicatorsV7
    
    indicators = ComprehensiveIndicatorsV7()
    df = indicators.compute_all(df)
"""

import numpy as np
import pandas as pd
from typing import List
import logging

logger = logging.getLogger(__name__)


class ComprehensiveIndicatorsV7:
    """
    V7: Domain knowledge features + additional indicators.
    
    New in V7:
    - Binary domain features (golden_cross, rsi_overbought, etc.)
    - Additional indicators (SAR, CMO, Donchian, CMF, HMA, etc.)
    - ~100 total features
    """
    
    def __init__(self):
        """Initialize the indicator computer."""
        try:
            import pandas_ta as ta
            self.ta = ta
            self.ta_available = True
        except ImportError:
            logger.warning("pandas_ta not installed. Run: pip install pandas_ta")
            self.ta_available = False
    
    def compute_all(self, df: pd.DataFrame) -> pd.DataFrame:
        """Compute all indicators + domain features."""
        df = df.copy()
        
        # Normalize column names
        required = ['Open', 'High', 'Low', 'Close', 'Volume']
        for col in required:
            if col not in df.columns:
                if col.lower() in df.columns:
                    df[col] = df[col.lower()]
                elif col.capitalize() in df.columns:
                    df[col] = df[col.capitalize()]
        
        if not self.ta_available:
            return self._compute_basic_indicators(df)
        
        # ===== MOVING AVERAGES =====
        df['sma_5'] = self.ta.sma(df['Close'], length=5)
        df['sma_10'] = self.ta.sma(df['Close'], length=10)
        df['sma_20'] = self.ta.sma(df['Close'], length=20)
        df['sma_50'] = self.ta.sma(df['Close'], length=50)
        df['sma_200'] = self.ta.sma(df['Close'], length=200)  # New for golden/death cross
        
        df['ema_5'] = self.ta.ema(df['Close'], length=5)
        df['ema_12'] = self.ta.ema(df['Close'], length=12)
        df['ema_26'] = self.ta.ema(df['Close'], length=26)
        
        df['vwma_20'] = self.ta.vwma(df['Close'], df['Volume'], length=20)
        df['kama'] = self.ta.kama(df['Close'], length=10)
        
        # New moving averages
        df['wma_20'] = self.ta.wma(df['Close'], length=20)
        df['hma_20'] = self.ta.hma(df['Close'], length=20)
        df['dema_20'] = self.ta.dema(df['Close'], length=20)
        
        zlma = self.ta.zlma(df['Close'], length=20)
        if zlma is not None:
            df['zlema_20'] = zlma
        
        # Ichimoku Cloud
        ichimoku = self.ta.ichimoku(df['High'], df['Low'], df['Close'])
        if ichimoku is not None and len(ichimoku) > 0 and ichimoku[0] is not None:
            for col in ichimoku[0].columns:
                df[f'ichimoku_{col}'] = ichimoku[0][col]
        
        # ===== MOMENTUM OSCILLATORS =====
        df['rsi_14'] = self.ta.rsi(df['Close'], length=14)
        df['rsi_7'] = self.ta.rsi(df['Close'], length=7)
        
        # CMO (Chande Momentum Oscillator) - New
        cmo = self.ta.cmo(df['Close'], length=14)
        if cmo is not None:
            df['cmo'] = cmo
        
        stoch = self.ta.stoch(df['High'], df['Low'], df['Close'])
        if stoch is not None:
            for col in stoch.columns:
                df[f'stoch_{col}'] = stoch[col]
        
        stochrsi = self.ta.stochrsi(df['Close'])
        if stochrsi is not None:
            for col in stochrsi.columns:
                df[f'stochrsi_{col}'] = stochrsi[col]
        
        # SMI (Stochastic Momentum Index) - New
        smi = self.ta.smi(df['Close'])
        if smi is not None:
            if isinstance(smi, pd.DataFrame):
                for col in smi.columns:
                    df[f'smi_{col}'] = smi[col]
            else:
                df['smi'] = smi
        
        macd = self.ta.macd(df['Close'])
        if macd is not None:
            for col in macd.columns:
                df[f'macd_{col}'] = macd[col]
        
        df['willr'] = self.ta.willr(df['High'], df['Low'], df['Close'])
        df['cci'] = self.ta.cci(df['High'], df['Low'], df['Close'])
        
        df['roc_10'] = self.ta.roc(df['Close'], length=10)
        df['roc_20'] = self.ta.roc(df['Close'], length=20)
        
        # DPO (Detrended Price Oscillator) - New
        dpo = self.ta.dpo(df['Close'], length=20)
        if dpo is not None:
            df['dpo'] = dpo
        
        ppo = self.ta.ppo(df['Close'])
        if ppo is not None:
            if isinstance(ppo, pd.DataFrame):
                for col in ppo.columns:
                    df[f'ppo_{col}'] = ppo[col]
            else:
                df['ppo'] = ppo
        
        trix = self.ta.trix(df['Close'])
        if trix is not None:
            if isinstance(trix, pd.DataFrame):
                for col in trix.columns:
                    df[f'trix_{col}'] = trix[col]
            else:
                df['trix'] = trix
        
        df['ao'] = self.ta.ao(df['High'], df['Low'])
        
        aroon = self.ta.aroon(df['High'], df['Low'])
        if aroon is not None:
            for col in aroon.columns:
                df[f'aroon_{col}'] = aroon[col]
        
        df['coppock'] = self.ta.coppock(df['Close'])
        
        kst = self.ta.kst(df['Close'])
        if kst is not None:
            for col in kst.columns:
                df[f'kst_{col}'] = kst[col]
        
        df['psl'] = (df['Close'] > df['Close'].shift(1)).rolling(12).mean() * 100
        
        # ===== VOLUME INDICATORS =====
        df['mfi'] = self.ta.mfi(df['High'], df['Low'], df['Close'], df['Volume'])
        df['bop'] = self.ta.bop(df['Open'], df['High'], df['Low'], df['Close'])
        
        # CMF (Chaikin Money Flow) - New
        cmf = self.ta.cmf(df['High'], df['Low'], df['Close'], df['Volume'])
        if cmf is not None:
            df['cmf'] = cmf
        
        pvo = self.ta.pvo(df['Volume'])
        if pvo is not None:
            if isinstance(pvo, pd.DataFrame):
                for col in pvo.columns:
                    df[f'pvo_{col}'] = pvo[col]
            else:
                df['pvo'] = pvo
        
        df['obv'] = self.ta.obv(df['Close'], df['Volume'])
        
        # AD (Accumulation/Distribution) - New
        ad = self.ta.ad(df['High'], df['Low'], df['Close'], df['Volume'])
        if ad is not None:
            df['ad'] = ad
        
        df['volume_sma_20'] = self.ta.sma(df['Volume'], length=20)
        df['volume_ratio'] = df['Volume'] / df['volume_sma_20'].replace(0, np.nan)
        
        df['rvgi'] = (df['Close'] - df['Open']) / (df['High'] - df['Low']).replace(0, np.nan)
        
        # ===== VOLATILITY INDICATORS =====
        bbands = self.ta.bbands(df['Close'])
        if bbands is not None:
            for col in bbands.columns:
                df[f'bb_{col}'] = bbands[col]
        
        # Donchian Channels - New
        donchian = self.ta.donchian(df['High'], df['Low'])
        if donchian is not None:
            for col in donchian.columns:
                df[f'donchian_{col}'] = donchian[col]
        
        df['atr'] = self.ta.atr(df['High'], df['Low'], df['Close'])
        df['atr_pct'] = df['atr'] / df['Close'] * 100
        
        df['true_range'] = self.ta.true_range(df['High'], df['Low'], df['Close'])
        df['mstd_20'] = self.ta.stdev(df['Close'], length=20)
        df['chop'] = self.ta.chop(df['High'], df['Low'], df['Close'])
        
        # ===== TREND STRENGTH =====
        adx = self.ta.adx(df['High'], df['Low'], df['Close'])
        if adx is not None:
            for col in adx.columns:
                df[f'adx_{col}'] = adx[col]
        
        # Parabolic SAR - New
        psar = self.ta.psar(df['High'], df['Low'], df['Close'])
        if psar is not None:
            if isinstance(psar, pd.DataFrame):
                for col in psar.columns:
                    df[f'psar_{col}'] = psar[col]
        
        df['bull_power'] = df['High'] - self.ta.ema(df['Close'], length=13)
        df['bear_power'] = df['Low'] - self.ta.ema(df['Close'], length=13)
        
        # ===== PRICE FEATURES =====
        df['return_1d'] = df['Close'].pct_change()
        df['return_5d'] = df['Close'].pct_change(5)
        df['return_10d'] = df['Close'].pct_change(10)
        df['return_20d'] = df['Close'].pct_change(20)
        df['log_return'] = np.log(df['Close'] / df['Close'].shift(1))
        df['high_low_pct'] = (df['Close'] - df['Low']) / (df['High'] - df['Low']).replace(0, np.nan)
        df['gap'] = (df['Open'] - df['Close'].shift(1)) / df['Close'].shift(1)
        df['intraday_range'] = (df['High'] - df['Low']) / df['Open']
        
        # ===== V7: DOMAIN KNOWLEDGE BINARY FEATURES =====
        df = self._compute_domain_features(df)
        
        # Drop intermediate columns
        if 'volume_sma_20' in df.columns:
            df = df.drop(columns=['volume_sma_20'])
        
        return df
    
    def _compute_domain_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Add domain knowledge binary/categorical features."""
        
        # Moving Average Crossovers
        if 'sma_50' in df.columns and 'sma_200' in df.columns:
            sma_50 = df['sma_50']
            sma_200 = df['sma_200']
            df['golden_cross'] = ((sma_50 > sma_200) & (sma_50.shift(1) <= sma_200.shift(1))).astype(float)
            df['death_cross'] = ((sma_50 < sma_200) & (sma_50.shift(1) >= sma_200.shift(1))).astype(float)
            df['above_sma_200'] = (df['Close'] > sma_200).astype(float)
        
        if 'sma_20' in df.columns:
            df['above_sma_20'] = (df['Close'] > df['sma_20']).astype(float)
        
        if 'ema_12' in df.columns and 'ema_26' in df.columns:
            df['ema_bullish'] = (df['ema_12'] > df['ema_26']).astype(float)
            df['ema_crossover_up'] = ((df['ema_12'] > df['ema_26']) & 
                                       (df['ema_12'].shift(1) <= df['ema_26'].shift(1))).astype(float)
        
        # RSI Zones
        if 'rsi_14' in df.columns:
            df['rsi_overbought'] = (df['rsi_14'] > 70).astype(float)
            df['rsi_oversold'] = (df['rsi_14'] < 30).astype(float)
            df['rsi_bullish'] = (df['rsi_14'] > 50).astype(float)
        
        # MACD Signals
        if 'macd_MACD_12_26_9' in df.columns and 'macd_MACDs_12_26_9' in df.columns:
            macd_line = df['macd_MACD_12_26_9']
            macd_signal = df['macd_MACDs_12_26_9']
            df['macd_bullish'] = (macd_line > macd_signal).astype(float)
            df['macd_crossover_up'] = ((macd_line > macd_signal) & 
                                        (macd_line.shift(1) <= macd_signal.shift(1))).astype(float)
            df['macd_positive'] = (macd_line > 0).astype(float)
        
        # Bollinger Band Signals
        bb_upper_cols = [c for c in df.columns if 'bb_' in c.lower() and 'upper' in c.lower()]
        bb_lower_cols = [c for c in df.columns if 'bb_' in c.lower() and 'lower' in c.lower()]
        if bb_upper_cols and bb_lower_cols:
            bb_upper = df[bb_upper_cols[0]]
            bb_lower = df[bb_lower_cols[0]]
            df['bb_upper_touch'] = (df['Close'] >= bb_upper * 0.99).astype(float)
            df['bb_lower_touch'] = (df['Close'] <= bb_lower * 1.01).astype(float)
            bb_width = bb_upper - bb_lower
            df['bb_squeeze'] = (bb_width < bb_width.rolling(20).quantile(0.2)).astype(float)
        
        # Volume Confirmation
        if 'volume_ratio' in df.columns:
            df['volume_surge'] = (df['volume_ratio'] > 1.5).astype(float)
            df['high_volume_up'] = ((df['Close'] > df['Close'].shift(1)) & 
                                     (df['volume_ratio'] > 1.5)).astype(float)
            df['high_volume_down'] = ((df['Close'] < df['Close'].shift(1)) & 
                                       (df['volume_ratio'] > 1.5)).astype(float)
        
        # ADX Trend Strength
        adx_cols = [c for c in df.columns if c.startswith('adx_') and 'ADX' in c]
        if adx_cols:
            adx_val = df[adx_cols[0]]
            df['strong_trend'] = (adx_val > 25).astype(float)
            df['weak_trend'] = (adx_val < 20).astype(float)
        
        # Multi-indicator confirmations
        bullish_count = 0
        if 'rsi_bullish' in df.columns:
            bullish_count = bullish_count + df['rsi_bullish']
        if 'macd_bullish' in df.columns:
            bullish_count = bullish_count + df['macd_bullish']
        if 'above_sma_20' in df.columns:
            bullish_count = bullish_count + df['above_sma_20']
        if 'ema_bullish' in df.columns:
            bullish_count = bullish_count + df['ema_bullish']
        df['bullish_score'] = bullish_count / 4  # 0-1 normalized
        
        return df
    
    def _compute_basic_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Fallback when pandas_ta is not available."""
        logger.warning("Using basic indicators only (pandas_ta not installed)")
        
        df['sma_5'] = df['Close'].rolling(5).mean()
        df['sma_20'] = df['Close'].rolling(20).mean()
        df['sma_50'] = df['Close'].rolling(50).mean()
        df['sma_200'] = df['Close'].rolling(200).mean()
        
        df['ema_12'] = df['Close'].ewm(span=12).mean()
        df['ema_26'] = df['Close'].ewm(span=26).mean()
        
        df['macd'] = df['ema_12'] - df['ema_26']
        df['macd_signal'] = df['macd'].ewm(span=9).mean()
        
        delta = df['Close'].diff()
        gain = delta.where(delta > 0, 0).rolling(14).mean()
        loss = (-delta.where(delta < 0, 0)).rolling(14).mean()
        rs = gain / loss.replace(0, np.nan)
        df['rsi_14'] = 100 - (100 / (1 + rs))
        
        df['return_1d'] = df['Close'].pct_change()
        df['volume_ratio'] = df['Volume'] / df['Volume'].rolling(20).mean()
        
        # Domain features
        df = self._compute_domain_features(df)
        
        return df
    
    def get_indicator_columns(self, df: pd.DataFrame) -> List[str]:
        """Get list of indicator column names."""
        exclude = ['Open', 'High', 'Low', 'Close', 'Volume', 'Date', 'date', 
                   'Adj Close', 'Dividends', 'Stock Splits', 'ticker', 'Ticker',
                   'trend', 'return_next', 'actual_return', 'actual_trend']
        return [col for col in df.columns if col not in exclude]
    
    def get_indicator_count(self, df: pd.DataFrame) -> int:
        """Get the number of indicator columns."""
        return len(self.get_indicator_columns(df))
